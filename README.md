...
()     正则中的分组
(?:)   只匹配不捕获
(?=)   正向预查,匹配满足正则条件的字符并且不捕获
(?!)   负向预查,匹配不满足正则条件的字符并且不捕获
```
```
3.普通元字符，代表本身含义
...
/*
* 规则分析   
'https://www.baidu.com/s?tn=43_hao_pg&isource=infinity#img'
 {tn:'43_hao_pg',isource:'infinity',HASH:'img'}
* 1.‘=’进行分割
* 2.‘=’左右侧用‘?=&#’四种字符分割
* 3.最后可能有‘#’hash值，可能没有
*/
...
let str = 'https://www.baidu.com/s?tn=43_hao_pg&isource=infinity#img'
queryParams(str)
```
7.将数字转为千分位
```
...
'13632674313'
 '13,632,674,313'
* 1.从后往前，每隔三位加个逗号
* 2.‘?=’,正向预查，匹配满足正则条件的字符不捕获
*/
let str = '13632674313'
str = str.replace(/\d{1,3}(?=(\w{3})+$)/g,function(big){
  return big + ','
})
```
***
...
> 当捕获不到内容后，捕获结果为null，会将lastIndex值设为0  
  ...
  ***
  ...
  > 3.'?:':只匹配不捕获  
  > 4.'?=':正向预查,匹配满足正则条件的字符并且不捕获 
  > 5.'?!':负向预查,匹配不满足正则条件的字符并且不捕获 
